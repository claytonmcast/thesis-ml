...
fn vec_vec_to_array2(vec_vec: Vec<Vec<f64>>) -> Result<Array2<f64>, &'static str> {
    if vec_vec.is_empty() {
        return Err("Input Vec<Vec<f64>> is empty.");
    }

    let rows = vec_vec.len();
    let cols = vec_vec[0].len(); // Assuming all inner Vecs have the same length

    // Check if inner Vecs have consistent lengths
    for inner_vec in &vec_vec {
        if inner_vec.len() != cols {
            return Err("Inner Vecs have inconsistent lengths.");
        }
    }

    let flattened_vec: Vec<f64> = vec_vec.into_iter().flatten().collect();

    match Array2::from_shape_vec((rows, cols), flattened_vec) {
        Ok(array) => Ok(array),
        Err(_) => Err("Failed to create Array2."),
    }
}
...
pub async fn fetch_and_train(
    ...
    let ndfeatures = vec_vec_to_array2(normalized_features_x.clone())?;
    let ndtarget = Array1::from_vec(target.clone());
    let num_epochs = 200;
    let learning_rate = 0.01;

    let num_samples = ndfeatures.nrows() as f64;
    let mut theta_0 = 0.0;
    let mut theta_1 = 0.0;
    ...
    for _epoch in 0..num_epochs {
        // Predictions
        let predictions = ndfeatures.map(|x| x * theta_1).sum_axis(Axis(1)) + theta_0;

        // Errors
        let errors = predictions.clone() - &ndtarget;

        // Gradients
        let d_theta_0 = errors.sum() / num_samples;
        let d_theta_1 = ndfeatures.t().dot(&errors).sum() / num_samples;
        ...
        let mse = errors.mapv(|e| e.powi(2)).sum() / (2.0 * num_samples);
